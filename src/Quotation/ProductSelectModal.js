...
class ProductSelectModal extends Component {
    ...
    renderFooter = () => {
        let { isLoading, productSelectView } = this.state;

        return (
            <>
                {!productSelectView && <button
                    type="button"
                    className="btn btn-secondary"
                    data-dismiss="modal"
                    onClick={this.onClickClose}
                    // onClick={() => this.setState({productSelectView: true})}
                >
                    Cancel
                </button>}
                {!productSelectView && <button
    ...
    onCloseTable = () => {
        ...
        if(Array.isArray(selectedRowKeys)){
            ...
            selectedRowKeys.map((o)=>{
                selectedItems[o] = {};
                if(items[o] === undefined){
                    items[o] = {};
                }
                if(product[o] === undefined){
                    product[o] = {}
                }
                // console.log(product[o.value])
                selectedItems[o].name = items[o].name ? items[o].name : product[o].name;
                selectedItems[o].hsn = items[o].hsn ? items[o].hsn : product[o].hsn;
                selectedItems[o].uom = items[o].uom ? items[o].uom : product[o].uom;
                selectedItems[o].qty = items[o].qty ? items[o].qty : product[o].qty;
                selectedItems[o].sRate = items[o].sRate ? items[o].sRate : product[o].rate;
                selectedItems[o].eRate = items[o].eRate ? items[o].eRate : product[o].eRate;
                // selectedItems[o.value].gst = items[o.value].gst !== undefined ? items[o.value].gst : product[o.value].gst;
            })
            this.setState({items: selectedItems})
            this.setState({selectedProduct: this.state.selectedRowKeys.map(o=>{
                    // console.log(product[o].name)
                    return {
                        value: o,
                        label: product[o].name
                    }
                }), productSelectView: false})
        }
    ...
    render() {
        ...
        const array = product ? Object.values(product).length > 0 && Object.values(product) : [];
        ...
        return (
            ...
            >
                <form>
                    {!productSelectView && <div className="row">
                    ...
                    { productSelectView && <div className={"col"}>
                        <div className="col-xl-12 col-12 col-md-12">
                            <Table
                            ...
                            {/*{product && Object.values(product).length > 0 && Object.values(product).map((o, i)=> {*/}
                            {/*    return (*/}
                    ...
                    {selectedProduct && !productSelectView && Array.isArray(selectedProduct) && selectedProduct.map((o, i)=>{
                        return (
...
export default connect(mapStateToProps, mapDispatchToProps)(ProductSelectModal);
